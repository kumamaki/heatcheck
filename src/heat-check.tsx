...
import { collectStats, installIStats, isIStatsInstalled, type ProcessStat, type ThermalStats } from "./system";
...
function diagnosisText(stats: ThermalStats): string {
  ...
  if (stats.thermalPressure === "critical") {
    return top ? `Critical — ${top.name} at ${top.cpu.toFixed(0)}% CPU` : "Critical thermal pressure";
  }
  if (stats.thermalPressure === "heavy") {
    return top ? `${top.name} is overloading your CPU (${top.cpu.toFixed(0)}%)` : "Heavy thermal load";
  }
  if (top && top.cpu >= 30) {
...
function ProcessItem({ proc, onRefresh }: { proc: ProcessStat; onRefresh: () => void }) {
  return (
    <List.Item
      title={proc.name}
      ...
      accessories={[
        ...
        {
          text: proc.memMB >= 1024 ? `${(proc.memMB / 1024).toFixed(1)} GB` : `${proc.memMB} MB`,
          tooltip: "Memory (RSS)",
        },
      ]}
      actions={
...
export default function HeatCheck() {
  ...
  async function promptInstallIStats() {
    const ok = await confirmAlert({
      ...
      message: "iStats is a Ruby gem that reads fan RPM and CPU temperature without sudo.\n\nInstall via: gem install iStats --user-install",
      primaryAction: { title: "Install", style: Alert.ActionStyle.Default },
      dismissAction: { title: "Skip" },
    });
    if (!ok) return;

    const toast = await showToast({ style: Toast.Style.Animated, title: "Installing iStats…" });
    try {
      await installIStats();
  ...
  const refreshActions = (
    <ActionPanel>
      <Action title="Refresh" icon={Icon.RotateClockwise} shortcut={{ modifiers: ["cmd"], key: "r" }} onAction={() => load(iStatsAvailable)} />
      {!iStatsAvailable && !checkingIStats && (
        <Action title="Install iStats (Fan & Temp Data)" icon={Icon.Download} onAction={promptInstallIStats} />
      )}
    </ActionPanel>
  );

  if (loading || stats === null) {
    return <List isLoading navigationTitle="Heat Check" searchBarPlaceholder="" />;
  }

  return (
    <List navigationTitle="Heat Check" searchBarPlaceholder="Filter processes…" actions={refreshActions}>
      {/* ── diagnosis ── */}
      <List.Section>
        <List.Item
          title={diagnosisText(stats)}
          subtitle={[
            stats.fanRpm != null ? `Fan ${stats.fanRpm.toLocaleString()} RPM` : null,
          ...
          accessories={[
            {
              tag: { value: capitalize(stats.thermalPressure), color: PRESSURE_COLOR[stats.thermalPressure] },
      ...
      <List.Section title="System">
        {stats.fanRpm != null ? (
          <List.Item
            title="Fan Speed"
            icon={{ source: Icon.Wind, tintColor: stats.fanRpm > 3500 ? Color.Orange : Color.PrimaryText }}
          ...
          <List.Item
            ...
            actions={
              <ActionPanel>
                <Action title="Install iStats" icon={Icon.Download} onAction={promptInstallIStats} />
                <Action title="Refresh" icon={Icon.RotateClockwise} shortcut={{ modifiers: ["cmd"], key: "r" }} onAction={() => load(iStatsAvailable)} />
              </ActionPanel>
            }
          />
        ...
        {stats.cpuTempC != null && (
          <List.Item
            ...
            icon={{ source: Icon.Temperature, tintColor: PRESSURE_COLOR[stats.thermalPressure] }}
        ...
        <List.Item
          ...
          icon={{ source: Icon.CircleFilled, tintColor: PRESSURE_COLOR[stats.thermalPressure] }}
          accessories={[
            { tag: { value: capitalize(stats.thermalPressure), color: PRESSURE_COLOR[stats.thermalPressure] } },
          ]}
          actions={refreshActions}
        ...
        <List.Item
          ...
          icon={{ source: Icon.MemoryChip, tintColor: MEM_PRESSURE_COLOR[stats.memoryPressure] }}
          accessories={[
            { tag: { value: capitalize(stats.memoryPressure), color: MEM_PRESSURE_COLOR[stats.memoryPressure] } },
          ]}
          actions={refreshActions}
        />
      </List.Section>
      ...
      <List.Section title="Top Processes">
        {stats.topProcesses.map((proc) => (
          <ProcessItem key={proc.pid} proc={proc} onRefresh={() => load(iStatsAvailable)} />
        ))}
      </List.Section>
    </List>
  );
