...
async function getTopProcesses(): Promise<ProcessStat[]> {
  const { stdout } = await execa("ps", ["-Ao", "pid=,comm=,pcpu=,rss=", "-r"]);
...
async function getIStatsData(): Promise<{ fanRpm: number | null; cpuTempC: number | null }> {
  try {
...
function deriveThermalPressure(cpuTempC: number | null, topCpu: number): ThermalPressure {
  if (cpuTempC !== null) {
...
export async function collectStats(withIStats: boolean): Promise<ThermalStats> {
  const [processes, memoryPressure, istats] = await Promise.all([
    getTopProcesses(),
    getMemoryPressure(),
    withIStats ? getIStatsData() : Promise.resolve({ fanRpm: null, cpuTempC: null }),
  ]);

  return {
    fanRpm: istats.fanRpm,
    cpuTempC: istats.cpuTempC,
    thermalPressure: deriveThermalPressure(istats.cpuTempC, processes[0]?.cpu ?? 0),
    memoryPressure,
...
export function formatStatsForAI(stats: ThermalStats): string {
  const lines: string[] = [
    ...
    ...stats.topProcesses.slice(0, 8).map((p) => `  ${p.name} (PID ${p.pid}): ${p.cpu}% CPU, ${p.memMB} MB RAM`),
  ];
  return lines.join("\n");
}
